...
void lcd_drawline(unsigned short x1, unsigned short y1, unsigned short x2, unsigned short y2)
{
	...
	{  
		...
		lcd_draw_point(uRow, uCol, lcd_tft_dev.forecolor);/* 画点 */
		xerr += delta_x ; 
		yerr += delta_y ; 
		if(xerr > distance) 
		{ 
...
void lcd_draw_Circle(unsigned short x0,unsigned short y0,unsigned char r)
{
    ...
    {
        lcd_draw_point(x  + mx, y  + my, lcd_tft_dev.forecolor);
        lcd_draw_point(y  + mx, x  + my, lcd_tft_dev.forecolor);
        lcd_draw_point(-x + mx, y  + my, lcd_tft_dev.forecolor);
        lcd_draw_point(-y + mx, x  + my, lcd_tft_dev.forecolor);
...
{  							  
	...
	{   
		...
		for(t1 = 0; t1 < 8; t1++)
		{			    
			if(temp & 0x80)lcd_draw_point(x, y, lcd_tft_dev.forecolor);
			else if(mode==0)lcd_draw_point(x, y, lcd_tft_dev.backcolor);
			temp <<= 1;
			y++;
			if(y >= lcd_tft_dev.height) return;			/* 超区域了 */	
			if((y - y0) == size)
			{
				y = y0;
				x++;
				if(x >= lcd_tft_dev.width) return;		/* 超区域了 */
				break;
			}
		}  	 
	}  		    	   	 	  
